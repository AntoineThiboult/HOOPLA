...
function pop = RunCCE(pop, params)
    ...
    for it = 1:beta
        ...
        for k=1:alpha
            ...
            ReflectionSol.Cost = CostFunction(ReflectionSol.Position, userdata);
            ...
            if ReflectionSol.Cost<B(end).Cost
                ...
                ContractionSol.Cost = CostFunction(ContractionSol.Position, userdata);
                
                if ContractionSol.Cost<B(end).Cost
                    B(end) = ContractionSol;
                else
                    B(end).Position = unifrnd(LowerBound, UpperBound);
                    B(end).Cost = CostFunction(B(end).Position, userdata);
                end
            end
